...
export class TasksService {
  ...
  async topic() {
    try {
      ...
      const categoryCodes = await this.codeItemService.findOneByCode(
        categoryCode,
      );
      const categoryCodeRandomNumber = Math.floor(
        Math.random() * categoryCodes.length,
      );
      ...
      const topicCodes = await this.codeItemService.findOneByCodeId(
        findCategoryCode.id,
      );
      const topicCodeRandomNumber = Math.floor(
        Math.random() * topicCodes.length,
      );
      ...
      const createNoticeBoard = await this.noticeBoardService.create(
